...
export const useFieldConfigStore = create(
    (set, get) => ({
        ...
        loadFieldConfigs: async () => {
            set({ isLoading: true })
            try {
                const response = await fetch('/api/field-configs/registro-civil')
                if (response.ok) {
        ...
        updateField: async (fieldId, updates) => {
            ...
            try {
                ...
                // Prepare updates with proper JSON stringification for options
                const preparedUpdates = { ...updates }
                if (preparedUpdates.options) {
                    preparedUpdates.options = JSON.stringify(preparedUpdates.options)
                }
                
                const response = await fetch(`/api/field-configs/registro-civil/${fieldId}`, {
                    method: 'PUT',
                    headers: {
        ...
        reorderFields: async (startIndex, endIndex) => {
            ...
            try {
                ...
                if (fieldOrders.length > 0) {
                    await fetch('/api/field-configs/registro-civil/reorder', {
                        method: 'PUT',
                        headers: {
                            'Content-Type': 'application/json',
                        },
            ...
            try {
                const response = await fetch('/api/field-configs/registro-civil', {
                    ...
                    body: JSON.stringify({
                        ...
                        options: JSON.stringify(fieldData.options || []),
