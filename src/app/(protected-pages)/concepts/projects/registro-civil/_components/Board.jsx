    ...
    const onDragEnd = async (result) => {
        ...
        if (result.destination && result.destination.droppableId === 'trash-can') {
            ...
            if (currentView === 'em-andamento') {
                ...
                try {
                    const response = await fetch(`/api/projects/registro-civil/${removedCard.id}`, {
                        method: 'DELETE',
                    });
        ...
        if (result.type === 'COLUMN') {
            if (currentView === 'em-andamento') {
                ...
                try {
                    const response = await fetch('/api/projects/registro-civil/reorder-boards', {
                        method: 'PUT',
                        headers: {
        ...
        if (currentView === 'em-andamento') {
            ...
            if (source.droppableId !== destination.droppableId) {
                ...
                if (movedCard) {
                    ...
                    try {
                        const response = await fetch(`/api/projects/registro-civil/${movedCard.id}`, {
                            method: 'PUT',
                            headers: {
                                'Content-Type': 'application/json',
                            },
