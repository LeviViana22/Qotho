...
export const useRegistroCivilStore = create((set, get) => ({
    ...initialState,
    ...
    deleteTicket: async (ticketId, currentUser = null) => {
        ...
        try {
            const response = await fetch(`/api/projects/registro-civil/${ticketId}`, {
                method: 'DELETE',
            });
            ...
            // Project deleted from database successfully
            } catch (error) {
    ...
    loadBoardMembers: async () => {
        try {
            ...
            if (response.ok) {
                ...
                // Board members loaded from database
            } else {
                // Registro civil members API not available, using empty array
                // If API fails, just use empty array - real users will be loaded by useRegistroCivilUsers
                set({ boardMembers: [] })
            }
        } catch (error) {
    ...
    saveBoardMembers: async (members) => {
        try {
            ...
            if (response.ok) {
                // Board members saved to database
            } else {
